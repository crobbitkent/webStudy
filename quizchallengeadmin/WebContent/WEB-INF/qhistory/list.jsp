...
<!-- paging start -->
  <ul class="pagination justify-content-center">
    ...
    <li class="page-item">
      <a class="page-link" href="/qhistory/list?page=1&perSheet=${pageMaker.pageInfo.perSheet}" aria-label="Previous">
    ...
    <li class="page-item">
      <a class="page-link" href="/qhistory/list?page=${pageMaker.start - 1}&perSheet=${pageMaker.pageInfo.perSheet}">Previous</a>
    </li>
    </c:if>

    <c:forEach begin="${pageMaker.start}" end="${pageMaker.end}" var="num"> 
    <li class="page-item ${num == pageMaker.pageInfo.page?'active':'' }">   
    <a class="page-link" href="/qhistory/list?page=${num}&perSheet=${pageMaker.pageInfo.perSheet}">${num}</a></li>
 	</c:forEach>
 	
 	<c:if test="${pageMaker.next == true}">
    <li class="page-item">
      <a class="page-link" href="/qhistory/list?page=${pageMaker.end + 1}&perSheet=${pageMaker.pageInfo.perSheet}">Next</a>
    </li>
    </c:if>
   
   <c:if test="${pageMaker.lastPage > pageMaker.pageInfo.page}">
    <li class="page-item">
      <a class="page-link" href="/qhistory/list?page=${pageMaker.lastPage}&perSheet=${pageMaker.pageInfo.perSheet}" aria-label="Next">
...
<select
	onChange="self.location='/qhistory/list?page=1&perSheet='+this.value">
	<option value="--">----</option>
